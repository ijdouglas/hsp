...
var welcome = {
    ...
    stimulus: `Welcome to the Human Simulation Paradigm for Verbs.<p>Press any key to begin.</p>`
};
...
var instructions2 = {
    ...
    stimulus: `
    <h1>Instructions</h1>
    <p align="justify">You will be asked to watch some short videos of parents playing toys with their children. These videos are action verb naming instances selected from the toy-play interaction, which means that parent in the video named an action verb. The sound of each video is computer generated, and a beep is inserted at the onset of the verb that parent named. Your job is to carefully watch the video and then guess the intended verb at the moment of parent naming indicated by the beep.</p>
...
var inst1 = [`
    <h1>Instructions</h1>
    <p align="justify">You will be asked to watch some short videos of parents playing toys with their children. These videos are action verb naming instances selected from the toy-play interaction, which means that the parent in the video named an action verb. The sound of each video is computer generated, and a beep is inserted at the onset of the verb that parent named. Your job is to carefully watch the video and then guess the verb that parents meant to say, indicated at the beep.</p>`];
...
var inst4 = [`<h1>Instructions (cont)</h1>
    <p align="justify">The study session will last 20 min, please make sure you have enough time to finish the study in one session. There will be no breaks in between.</p>
...
var inst1_baseline_ch = [`
    <h1>指示</h1>
    <p align="justify">在此實驗中，我們將會播放一些父母和小孩玩玩具的影片。每個影片中父母都對孩子説了一個動詞。我們將每個影片的原聲抹去，但在父母説出動詞的瞬間您會聽到嗶聲。您的任務就是通過觀看影片，猜出父母對孩子説了哪個動詞。</p>`];

var inst2_baseline_ch = [`
    <h1>指示</h1>
    <p align="justify">請注意這些動詞都是描述具體動作的動詞，例如 “跳”和“鼓掌”。請不要猜測抽象的，無法用具體動作表示的動詞，例如 “想”、“看”、“做” 等。`];

var inst3_baseline_ch = [`
    <h1>指示</h1>
    <p align="justify">每個影片只會播放一次，請務必看完完整影片后再輸入動詞。影片播完之後，您會有二十秒輸入您的答案。如果您不確定或不知道，也請在這二十秒之内輸入一個最有可能的答案。如果你沒有在二十秒之内輸入一個答案，下一個影片會開始自動播放。如果連續錯過5個影片，本實驗將會自動終止。</p>`];

var inst4_baseline_ch = [`<h1>指示</h1>
    <p align="justify">整個實驗需要約二十分鐘左右的時間。因為實驗過程中沒有休息時間，所以請您安排足夠的時間一次完成整個實驗。</p>
...
var completion_code_turk_ch = [`<p>謝謝您參加本實驗！</p> 請記下以下號碼： <b>IU200</b><p><b>您需要回到亞馬遜網頁輸入這個號碼。</p></b>`];
...
var end_turk_ch = [`<p>您的資料已經被自動保存，現在可以關閉網頁。</p> <p>請您不要忘記記下以下號碼: <b>IU200</b></p>並回到亞馬遜網頁輸入這個號碼`];
...
var instruct_block_baseline_ch = {
    type: "instructions",
    pages: [inst1_baseline_ch, inst2_baseline_ch, inst3_baseline_ch, inst4_baseline_ch],
    show_clickable_nav: true,
    timing_post_trial: transitionDelay
};
...
var end_block_turk_ch = {
    type: "instructions",
    pages: [completion_code_turk_ch, end_turk_ch],
    show_clickable_nav: true,
    timing_post_trial: transitionDelay
};
...
function validate_response(resp) {
    ...
    // if (!(resp.length > 0)) {
    //     window.alert("need at least 1 character")
    //     return false;
    // }

    if (!(/[\u3400-\u9FBF]/.test(resp.toString())) && resp.length != 0) {
        window.alert("請您輸入漢字")
    ...
    plugin.trial = function (display_element, trial) {
        ...
        ******************************************/
        ...
        for (var i = 0; i < trial.num_responses; i++) {
            ...
            html += '<input type="text" autocomplete="off" onfocusout="validate_response(document.getElementById(\'input-0\').value)" id="input-' + i + '"  name="#jspsych-survey-text-response-' + i + '" data-name="' + q.name + '" size="' + q.columns + '" ' + autofocus + ' ' + req + ' placeholder="' + q.placeholder + '" disabled></input><br>';
        }
