...
overall_Freq = Counter(overall)
print(overall_Freq)
df = pd.DataFrame.from_dict(overall_Freq, orient='index').reset_index()
df = df.rename(columns={'index':'word', 0:'count'})
df.to_csv("overall_counts.csv")
print()
for target in ["shake","hold","eat","fall","drive","turn","put","cut","fit","knock","stack"]:
    subset = overall_subset[target]
    print(target)
    subset_freq = Counter(subset)
    print(subset_freq)
    df = pd.DataFrame.from_dict(subset_freq, orient='index').reset_index()
    df = df.rename(columns={'index':'word', 0:'count'})
    df.to_csv(target+"_counts.csv")
    print()
